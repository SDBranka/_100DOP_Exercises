...
#Hint 6: Create a function called calculate_score() that takes a List
# of cards as input and returns the score. Look up the sum() function to 
# help you do this.
...
#  computer_score. If the computer and user both have the same score, then
#  it's a draw. If the computer has a blackjack (0), then the user loses. 
# If the user has a blackjack (0), then the user wins. If the user_score 
# is over 21, then the user loses. If the computer_score is over 21, then 
# the computer loses. If none of the above, then the player with the highest
#  score wins.
...
def play_blackjack():
    ...
    while play_blackjack == "y":
        ...
        # card to the user_cards List. If no, then the game has ended.
            ...
            user_draw_again = input("Take another card? (y/n) ")
            if user_draw_again == 'y':
                user_cards.append(deal_card())
            else:
                play_blackjack = 'n'

    #Hint 12: Once the user is done, it's time to let the computer play. The 
...
# clear the console and start a new game of blackjack and show the logo 
# from art.py.
